...
export class InvoicesService {
  ...
  async findOne(id: number, dateStart: string, dateEnd: string) {
    ...
    //  SELECT mov.numDocto, nota.codNumNota, nota.codSerie
    //  FROM Est.Movimento AS mov
    //  JOIN Fat.NotaFiscal AS nota ON mov.numDocto=STRING("NF",nota.codNumNota) AND mov.codEmpresa=nota.codEmpresa
    //  JOIN Fat.NotaFiscalItem AS item ON nota.codNumNota=item.numero AND nota.dataEmissao=item.dataEmissao
    //  WHERE mov.codEmpresa=2 AND mov.codItem IN (SELECT codItem FROM Cgi.MascSaida WHERE {fn LEFT(mascara,2)}="12") AND mov. dataMovto>=DATE("2023-12-01")
