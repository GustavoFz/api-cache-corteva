...
export class InvoicesService {
  ...
  async findOne(id: number, dateStart: string, dateEnd: string) {
    ...
    JOIN itemNotaEntrada AS item 
        ON item.codFornecedor=mov.codFornecedor AND item.codItem=mov.codItem AND item.numeroNota=mov.numeroNota AND item.codEmpresa=mov.codEmpresa AND mov.serieFiscal=item.serieNota AND mov.dataLancamento=item.dataEntrada AND mov.qtdItem=item.qtdItem AND item.vlrEntrada=mov.vlrUnitario
